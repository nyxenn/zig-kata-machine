...
// Json models
const JsonProperty = struct {
    name: []const u8,
    type: []const u8,
    scope: []const u8,
};

const JsonMethod = struct {
    name: []const u8,
    args: []const u8,
    return_type: []const u8,
};

const JsonPredefined = struct {
    name: []const u8,
...
const JsonConfiguration = struct { predefineds: []JsonPredefined, exercises: []JsonExercise };
...
export const Configuration = struct { predefineds: []Predefined, exercises: []Exercise };

fn parse_config(allocator: *const std.mem.Allocator) !void {
    const file = try std.fs.cwd().openFile("dsa.json");
    _ = file;
    _ = allocator;
}
